...
export function LogoutButton({ className, showText = true }: { className?: string; showText?: boolean }) {
  return (
    <button
      ...
      title="Logout"
      className={`${className || "inline-flex items-center gap-2 rounded-2xl bg-slate-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700"} cursor-pointer`}
    >
